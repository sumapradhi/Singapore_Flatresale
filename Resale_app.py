...
def predict_price(year,town,flat_type,flr_area_sqm,flat_model,rem_les_year,lese_coms_dt,str_str,
              str_end):
    ...
    with open(r"C:\GUVI\NewVM\SingaporeFlat_Resale\Resale_Flat_Prices_Model_1.pkl","rb") as f:
        regg_model= pickle.load(f)

    user_data = np.array([[year_1,town_2,flt_ty_2,flr_ar_sqm_1,
                           flt_model_2,rem_les_year,
                           lese_coms_dt,str_str,str_end]])
...
if select == "Home":
    ...
    with col2:
        ...
        re_les_year= st.number_input("Enter the Value of Remaining Lease Year (Min: 40 / Max: 100)")

        les_coms_dt= st.selectbox("Select the Lease_Commence_Date", [str(i) for i in range(1966,2022)])

    button= st.button("Predict the Price", use_container_width= True)

    if button:
        # Input validation
        if (28 <= flr_area_sqm <= 307) and (40 <= re_les_year <= 100): 
            pre_price= predict_price(year, town, flat_type, flr_area_sqm, flat_model,
                        stry_start, stry_end, re_les_year, les_coms_dt)

            st.write("## :green[**The Predicted Price is :**]",pre_price)

        else:
            st.error("Invalid inputs. Please ensure that the inputs are within the specified ranges.")
